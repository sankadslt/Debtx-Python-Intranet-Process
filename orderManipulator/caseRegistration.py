        ...
        try:
            ...
            for row in rows:
                ...
                if not self.mongo_data["Customer_Details"].get("Customer_Name"):
                    # Handle contact details - using exact structure from working program
                    if row.get("TECNICAL_CONTACT_EMAIL"):
                        logger.debug(f"Processing email: {row.get('TECNICAL_CONTACT_EMAIL')}, Valid: {row.get('TECNICAL_CONTACT_EMAIL') and '@' in row.get('TECNICAL_CONTACT_EMAIL')}")
                    ...
                    if row.get("MOBILE_CONTACT"):
                        logger.debug(f"Processing mobile: {row.get('MOBILE_CONTACT')}, Valid: {bool(row.get('MOBILE_CONTACT'))}")
                        contact_details_element = {
                            "Contact_Type": "mobile",
                    ...
                    if row.get("WORK_CONTACT"):
                        logger.debug(f"Processing work: {row.get('WORK_CONTACT')}, Valid: {bool(row.get('WORK_CONTACT'))}")
                        contact_details_element = {
                            "Contact_Type": "fix",
